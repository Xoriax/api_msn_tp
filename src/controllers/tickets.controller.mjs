...
export default class Tickets {
  ...
  purchaseTicket() {
    this.app.post(
      '/ticket-types/:ticketTypeId/purchase',
      authenticateToken,
      async (req, res) => {
        try {
          ...
          if (!event) {
            return res.status(404).json({
              code: 404,
              message: 'Événement associé au billet non trouvé'
            });
          }
      ...
      async (req, res) => {
        try {
          ...
          if (ticket.buyerInfo.email !== user.email) {
            return res.status(403).json({
              code: 403,
              message: 'Seul l\'acheteur du billet peut le consulter'
            });
          }
    ...
    this.app.post(
      ...
      async (req, res) => {
        try {
          ...
          const event = await this.EventModel.findById(ticket.event_id);
          if (!event) {
            return res.status(404).json({
              code: 404,
              message: 'Événement associé au billet non trouvé'
          ...
          if (!isCreator && !isOrganizer) {
            return res.status(403).json({
              code: 403,
              message: 'Seuls les créateurs et organisateurs de l\'événement peuvent marquer les billets comme utilisés'
            });
          }
  ...
  updateTicketType() {
    this.app.put(
      '/ticket-types/:id',
      authenticateToken,
      checkTicketTypeManageAccess(this.TicketTypeModel, this.EventModel),
      async (req, res) => {
        try {
          const { id } = req.params;
    ...
    this.app.delete(
      ...
      async (req, res) => {
        try {
          ...
          if (user.email !== ticket.buyerInfo.email) {
            return res.status(403).json({
              code: 403,
              message: 'Seul l\'acheteur du billet peut l\'annuler'
            });
          }
